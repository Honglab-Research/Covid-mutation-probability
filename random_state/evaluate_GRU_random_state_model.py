...
y_true = filtered_data['Nextstrain_clade'].map(clade_hierarchy.get)
...
# 모델 로드 및 예측 수행
model = tf.keras.models.load_model('models/GRU_random_state_model.h5')
gru_y_pred_labels = np.argmax(model.predict(np.expand_dims(X_filtered, axis=-1)), axis=1)

# 성능 평가 및 저장
metrics = {
    'Accuracy': accuracy_score(y_true_labels, gru_y_pred_labels),
    'Precision': precision_score(y_true_labels, gru_y_pred_labels, average='weighted'),
    'Recall': recall_score(y_true_labels, gru_y_pred_labels, average='weighted'),
    'F1 Score': f1_score(y_true_labels, gru_y_pred_labels, average='weighted')
}

with open('GRU_random_state_performance.txt', 'w') as f:
    for metric, value in metrics.items():
        f.write(f"GRU Model {metric}: {value:.4f}\n")

print("Performance metrics have been saved to GRU_random_state_performance.txt.")
