...
# Random Forest 모델 학습
def train_rf_model(X_train, y_train, model_path):
...
for scenario, train_data, test_data, eval_data in scenarios:
    ...
    model_path = f"models/Random_Forest_wave_{scenario.replace(' ', '_').replace('-', '').lower()}.joblib"
    train_rf_model(X_train, X_train, model_path)
    results.append(evaluate_model(X_test, X_eval, model_path, f"Random Forest {scenario}"))

# 결과를 데이터프레임으로 정리
results_df = pd.DataFrame(results, columns=["Model", "Accuracy", "Precision", "Recall", "F1 Score", "All Combinations"])

results_df.to_csv('Random_Forest_wave_performance.txt', sep='\t', index=False)
...
# 모든 조합을 파일로 저장
all_combinations_df.to_csv('wave_all_combinations_Random_Forest.txt', sep='\t', index=False)
print("All combinations saved to wave_all_combinations_Random_Forest.txt")
